...
Elm.Urls.make = function (_elm) {
   ...
   var searchDocuments = function (query) {    return A2($Basics._op["++"],"/search_documents/",query);};
   var image = function (s) {    return A2($Basics._op["++"],"/static/img/",s);};
   return _elm.Urls.values = {_op: _op,image: image,searchDocuments: searchDocuments,document: document,docsWithTag: docsWithTag};
};
...
Elm.Database.Tag.make = function (_elm) {
   ...
   return _elm.Database.Tag.values = {_op: _op,Tag: Tag,GRADE: GRADE,SBJCT: SBJCT,Error: Error,decoder: decoder,decodeKind: decodeKind,view: view};
};
Elm.Database = Elm.Database || {};
...
Elm.Database.Document.make = function (_elm) {
   ...
   var ratingView = function (doc) {
      ...
      _U.list([A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: A2($Basics._op["++"],$Basics.toString($Basics.round(doc.rating * 100)),"%")}
                                              ,{ctor: "_Tuple2",_0: "background-color",_1: "#f1c40f"}]))
              ,$Html$Attributes.$class("rating")]),
      _U.list([]))]));
   };
   var detailView = function (doc) {
      return _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("row rating-cont")]),_U.list([ratingView(doc)]))
                     ,A2($Html.div,_U.list([$Css.row]),A2($List.map,$Database$Tag.view(true),doc.tags))
                     ,dateView(doc)]);
   };
   var preview = function (doc) {
      ...
      _U.list([A2($Html.div,
              ...
              ,A2($Html.div,_U.list([$Css.column(4)]),detailView(doc))]))]));
   };
...
Elm.SharedView.make = function (_elm) {
   ...
   return _elm.SharedView.values = {_op: _op,header: header};
};
...
Elm.Server.make = function (_elm) {
   ...
   $Urls = Elm.Urls.make(_elm);
   var _op = {};
   var searchDocuments = function (query) {    return A2($Http.get,$Json$Decode.list($Database$Document.decoder),$Urls.searchDocuments(query));};
   return _elm.Server.values = {_op: _op,searchDocuments: searchDocuments};
};
...
Elm.Search.make = function (_elm) {
   ...
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Server = Elm.Server.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var NewSearchResults = function (a) {    return {ctor: "NewSearchResults",_0: a};};
   var getDocuments = function (text) {    return $Effects.task(A2($Task.map,NewSearchResults,$Task.toResult($Server.searchDocuments(text))));};
   var update = F2(function (action,model) {
      var _p0 = action;
      if (_p0.ctor === "InputChange") {
            var _p1 = _p0._0;
            return {ctor: "_Tuple2",_0: _U.update(model,{query: _p1}),_1: getDocuments(_p1)};
         } else {
            if (_p0._0.ctor === "Ok") {
   ...
   var Model = F2(function (a,b) {    return {query: a,documents: b};});
   var init = {ctor: "_Tuple2",_0: A2(Model,"",_U.list([])),_1: getDocuments("")};
