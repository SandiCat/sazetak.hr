...
Elm.DocumentDisplay = Elm.DocumentDisplay || {};
Elm.DocumentDisplay.make = function (_elm) {
   "use strict";
   _elm.DocumentDisplay = _elm.DocumentDisplay || {};
   if (_elm.DocumentDisplay.values) return _elm.DocumentDisplay.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Css = Elm.Css.make(_elm),
   ...
   "DocumentDisplay.InitalData",
   function (v) {
      return typeof v === "object" && "documentJson" in v ? {_: {}
                                                            ,documentJson: typeof v.documentJson === "string" || typeof v.documentJson === "object" && v.documentJson instanceof String ? v.documentJson : _U.badPort("a string",
                                                            v.documentJson)} : _U.badPort("an object with fields `documentJson`",v);
   });
   var markdownOptions = {githubFlavored: $Maybe.Just({tables: true,breaks: false}),defaultHighlighting: $Maybe.Nothing,sanitize: true,smartypants: true};
   var view = F2(function (address,model) {
      ...
      _U.list([$SharedView.header
              ,A2($Html.div,
              _U.list([$Css.row]),
              _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("eight columns document")]),_U.list([$Markdown.toHtml(model.document.content)]))
                      ,A2($Html.div,
                      _U.list([$Css.column(4)]),
                      _U.list([A2($Html.h4,_U.list([$Css.row]),_U.list([$Html.text(model.document.name)]))
                              ,A2($Html.p,_U.list([$Css.row]),_U.list([$Html.text(model.document.description)]))
                              ,$Database$Document.detailView(model.document)]))]))
              ,$SharedView.footer]));
   });
   var update = F2(function (action,model) {
      var _p0 = action;
      var _p2 = _p0._0;
      return $EffectsUtil.noFx(_U.update(model,
      {initialData: _p2
      ,document: function () {
         var _p1 = A2($Json$Decode.decodeString,$Database$Document.decoder,_p2.documentJson);
         if (_p1.ctor === "Ok") {
               return _p1._0;
            } else {
               return A2($Debug.log,$Basics.toString(_p1._0),$Database$Document.empty);
            }
      }()}));
   });
   ...
   var InitalData = function (a) {    return {documentJson: a};};
   var Model = F2(function (a,b) {    return {initialData: a,document: b};});
   var init = $EffectsUtil.noFx(A2(Model,InitalData(""),$Database$Document.empty));
   var init$ = function () {
      var update$ = F2(function (action,_p3) {
         var _p4 = _p3;
         var _p5 = A2(update,action,_p4._0);
         var model$ = _p5._0;
         var fx$ = _p5._1;
         return {ctor: "_Tuple2",_0: model$,_1: $Effects.batch(_U.list([_p4._1,fx$]))};
      });
   ...
   return _elm.DocumentDisplay.values = {_op: _op
                                        ,Model: Model
